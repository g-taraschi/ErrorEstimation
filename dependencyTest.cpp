...
TPZMultiphysicsCompMesh* createCompMeshMixed(
  TPZGeoMesh *gmesh, int orderFlux = 1, int orderPressure = 1);

// Create a computation mesh for mixed approximation with div sigma = 0
TPZMultiphysicsCompMesh* createCompMeshMixedKernel(TPZGeoMesh *gmesh, int orderFlux = 1);
...
int main (int argc, char * const argv[]) {
  ...
  gexact.fExact = TLaplaceExample1::EHarmonic; // For a problem with known solution
  ...
  int orderH1 = 1; // Polynomial orders
  int orderFlux = 1;
  int orderPressure = 1;
  ...
  while (iteration < 2) {
    ...
    TPZMultiphysicsCompMesh* cmeshMixed = createCompMeshMixed(gmesh, orderFlux, orderPressure);
    //TPZMultiphysicsCompMesh* cmeshMixed = createCompMeshMixedKernel(gmesh, orderFlux);
    TPZCompMesh* cmeshH1 = createCompMeshH1(gmesh, orderH1);
...
TPZMultiphysicsCompMesh* createCompMeshMixed(TPZGeoMesh *gmesh, int orderFlux, int orderPressure) {

  // --- Flux atomic cmesh ----

  TPZCompMesh *cmeshFlux = new TPZCompMesh(gmesh);
  cmeshFlux->SetDimModel(gmesh->Dimension());
  cmeshFlux->SetDefaultOrder(orderFlux);
  if (orderPressure < 1) {
  ...
  cmeshPressure->SetDefaultOrder(orderPressure);
  if (orderPressure < 1) {
...
TPZMultiphysicsCompMesh* createCompMeshMixedKernel(TPZGeoMesh *gmesh, int orderFlux) {
  ...
  cmeshFlux->SetDimModel(gmesh->Dimension());
  cmeshFlux->SetDefaultOrder(orderFlux);
  cmeshFlux->ApproxSpace().SetHDivFamily(HDivFamily::EHDivKernel);
  ...
  {
    std::ofstream out("cmeshKernel.txt");
    cmeshFlux->Print(out);
  }
  ...
  TPZCompMesh *cmeshPressure = new TPZCompMesh(gmesh);
  cmeshPressure->SetDimModel(gmesh->Dimension());
  cmeshPressure->SetDefaultOrder(1);
  // cmeshPressure->SetAllCreateFunctionsDiscontinuous();
  cmeshPressure->SetAllCreateFunctionsContinuous();
  cmeshPressure->ApproxSpace().CreateDisconnectedElements(true);
  ...
  if (gmesh->Dimension() == 3) {
    std::cout << "Only 2D case implemented" << std::endl;
    DebugStop();
  }
